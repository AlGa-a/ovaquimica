...
.cont-root { min-height:100%; background:#0B0620; }

/* Hero */
.hero { position:relative; padding:clamp(14px,4vw,20px) var(--page-px,20px) 0; overflow:hidden; background:#0D0825; border-bottom:1px solid rgba(124,58,237,0.2); }
.hero-bg { position:absolute; inset:0; pointer-events:none; }

/* Tabs — scroll horizontal en móvil */
.module-tabs { display:flex; gap:6px; margin-bottom:14px; position:relative; z-index:1; overflow-x:auto; padding-bottom:4px; scrollbar-width:none; -webkit-overflow-scrolling:touch; flex-wrap:nowrap; }
.module-tabs::-webkit-scrollbar { display:none; }
.module-tab { flex-shrink:0; display:flex; align-items:center; gap:5px; padding:5px 13px; border-radius:20px; border:1.5px solid rgba(124,58,237,0.3); background:rgba(124,58,237,0.08); color:#8B7BAF; font-size:clamp(.68rem,2vw,.73rem); font-weight:700; cursor:pointer; transition:all .2s; white-space:nowrap; font-family:'Nunito',sans-serif; min-height:32px; }
.module-tab:hover { border-color:rgba(124,58,237,0.5); color:#C4B5FD; }
...
.hero-title { font-family:'Poppins',sans-serif; font-weight:900; font-size:var(--fs-hero,1.5rem); color:#F1F5F9; margin:4px 0 6px; line-height:1.2; }
.hero-desc  { font-size:var(--fs-sm,.82rem); color:#8B7BAF; line-height:1.6; margin-bottom:10px; max-width:560px; }
.hero-meta  { display:flex; gap:12px; flex-wrap:wrap; }
.meta-item  { display:flex; align-items:center; gap:5px; font-size:.74rem; color:#8B7BAF; font-weight:600; }
.hero-prog-track { height:3px; background:rgba(124,58,237,0.15); margin:0 calc(-1*var(--page-px,20px)); }
.hero-prog-fill  { height:100%; transition:width .8s ease; min-width:3px; }

/* Videos grid */
.videos-section { padding:clamp(14px,3vw,20px) var(--page-px,20px) clamp(20px,5vw,28px); }
.videos-header  { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
.videos-title   { font-family:'Poppins',sans-serif; font-weight:800; font-size:var(--fs-md,.9rem); color:#F1F5F9; }
.videos-count   { font-size:.72rem; color:#8B7BAF; font-weight:600; background:rgba(124,58,237,0.15); padding:2px 10px; border-radius:20px; }

/* Cards */
.video-card { background:#140B33; border-radius:14px; border:1px solid rgba(124,58,237,0.2); overflow:hidden; transition:transform .22s ease,box-shadow .22s ease; }
@media (hover:hover) { .video-card:hover { transform:translateY(-3px); box-shadow:0 10px 28px rgba(0,0,0,0.4); border-color:rgba(124,58,237,0.4); } }
.card-thumb { position:relative; height:clamp(140px,40vw,170px); background:#0D0825; cursor:pointer; overflow:hidden; }
.thumb-img  { width:100%; height:100%; object-fit:cover; display:block; }
@media (hover:hover) { .card-thumb:hover .thumb-img { transform:scale(1.04); } .thumb-img { transition:transform .3s ease; } }
.play-overlay { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.3); }
.play-circle  { width:clamp(42px,12vw,52px); height:clamp(42px,12vw,52px); border-radius:50%; display:flex; align-items:center; justify-content:center; border:2px solid rgba(255,255,255,0.4); transition:transform .2s; }
...
.dur-badge { display:flex; align-items:center; gap:3px; background:rgba(0,0,0,0.55); border-radius:6px; padding:2px 6px; font-size:.58rem; color:rgba(255,255,255,.85); font-weight:700; }
.seen-badge { position:absolute; bottom:7px; right:7px; display:flex; align-items:center; gap:3px; background:#10B981; border-radius:6px; padding:2px 7px; font-size:.58rem; font-weight:800; color:white; }
.video-iframe { width:100%; height:100%; border:none; display:block; }
.card-body  { padding:clamp(10px,3vw,12px) clamp(10px,3vw,14px) 6px; }
.cap-label  { font-size:.56rem; font-weight:800; text-transform:uppercase; letter-spacing:1.2px; margin-bottom:3px; }
.card-title { font-family:'Poppins',sans-serif; font-weight:700; font-size:clamp(.82rem,2.5vw,.88rem); color:#F1F5F9; line-height:1.3; margin-bottom:4px; }
.card-desc  { font-size:clamp(.7rem,2vw,.76rem); color:#8B7BAF; line-height:1.5; }
.card-divider { height:1px; background:rgba(124,58,237,0.15); margin:0 12px; }
.card-actions { padding:8px 10px; display:flex; align-items:center; justify-content:space-between; }
.tts-btn { display:flex; align-items:center; gap:5px; background:rgba(124,58,237,0.12); border:1px solid rgba(124,58,237,0.25); border-radius:8px; padding:6px 10px; color:#C4B5FD; font-size:.72rem; font-weight:700; cursor:pointer; transition:all .18s; font-family:'Nunito',sans-serif; min-height:unset; }
.icon-btn { width:32px; height:32px; border-radius:8px; border:none; background:rgba(255,255,255,0.04); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background .15s; min-height:unset; }
</style>